...
interface YouCommonButtonProps extends Omit<ButtonProps, 'style' | 'className'> {
...
export function YouCommonButton({ leading, xstyle, isText, isElevated, isFilled, isTonal, isOutlined, children, text, ...props }: YouCommonButtonProps) {
  return (
    <Button
      style={(args) =>
        toStyle(
          ...
          stylex.props(
            ...
            (args.isDisabled || args.isPending) && (isElevated || isFilled || isText) ? styles.isDisabledBackground : null,
            (args.isDisabled || args.isPending) && isOutlined ? styles.isDisabledOutline : null,
            xstyle,
          ).style,
        )
      }
